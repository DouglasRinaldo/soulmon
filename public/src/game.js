...
// Listener: criatura morreu — continuar fluxo de batalha
setTimeout(function () {
  if (window.EventBus) {
    EventBus.on('creature:dead', function (data) {
      var c = data.creature;
      // Desabilita botão de captura
      var capBtn = document.getElementById('capbtn');
      if (capBtn) { capBtn.disabled = true; capBtn.style.opacity = '0.3'; }

      var nextInTeam = G.team.find(function (x) { return !x.dead; });
      if (nextInTeam) {
        setTimeout(function () {
          addLog('Escolha outra criatura.', 'sys');
          openSwap(true);
        }, 900);
        return;
      }
      ...
      if (hallAlive.length > 0) {
        setTimeout(function () {
          ...
          while (G.team.length < 3 && G.hall.length > 0) {
            var r = G.hall.shift();
            r.hp = Math.max(1, Math.floor(r.maxHp * 0.30));
            G.team.push(r);
          }
          G.activeIdx = 0;
          killS('my-wrap'); killS('en-wrap');
          document.getElementById('battle').style.display = 'none';
          showEscapedByHall(G.team.slice(0, 3));
        }, 1600);
      } else {
        setTimeout(function () {
          moveAllDeadToBook();
          var hero = G.hero;
          // Dungeon: callback especial
          if (G.battle && G.battle._dungCb) {
            var _dcb = G.battle._dungCb;
            G.battle._dungCb = null;
            document.getElementById('battle').style.display = 'none';
            try { _dcb(false); } catch (e) { wipeAndGameOver(); }
            return;
          }
          // Herói vivo: tela de última chance
          if (hero && hero.hp > 0 && typeof openSurvivalScreen === 'function') {
            var enemy = G.battle ? G.battle.enemy : null;
            document.getElementById('battle').style.display = 'none';
            if (enemy) {
              openSurvivalScreen(enemy);
            } else {
              wipeAndGameOver();
            }
          } else {
...
function spawnS(wid, c) {
  ...
  if (_tplForModel && _tplForModel.modelFile) {
    scene.add(new THREE.AmbientLight(0xffffff, 0.8));
    var softL = new THREE.DirectionalLight(0xffffff, 0.6);
    softL.position.set(0, 5, 5);
    scene.add(softL);
  }
  ...
  // ── NOVO: modelo GLB externo ──
  var _tplForModel = (typeof TPLS !== 'undefined') && TPLS.find(function (t) { return t.name === (c.tplName || c.name); });
  if (_tplForModel && _tplForModel.modelFile) {
    var _glbPlaceholder = new THREE.Group();
    mesh = _glbPlaceholder;
    scene.add(mesh);
    ModelLoader.spawnForBattle(c, scene, function (glbMesh) {
      if (glbMesh) scene.add(glbMesh);
    });
  } else
